...
- **Tradução assistida de rótulos em base de dados** (`label`): **DeepL API** no backend para rótulos de **field** (variáveis `DEEPL_API_KEY` / `DEEPL_API_BASE_URL`, header `Authorization`, `source_lang` + `target_lang` conforme `label_lang`, ajuste automático do host Free vs Pro quando a URL não corresponde à chave). Decisão e contrato em [skills/implementation/i18n/policy.md](../skills/implementation/i18n/policy.md).
