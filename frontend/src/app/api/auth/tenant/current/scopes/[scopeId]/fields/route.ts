...
export async function GET(
  ...
  const endpoint = `/auth/tenant/current/scopes/${scopeId}/fields${search}`;
  const result = await backendFetch<TenantScopeFieldDirectoryResponse>(endpoint, {
    method: "GET",
...
export async function POST(
  ...
  const result = await backendFetch<TenantScopeFieldDirectoryResponse>(
    `/auth/tenant/current/scopes/${scopeId}/fields`,
    {
      method: "POST",
      token: authResult.token,
      body
