...
export async function GET(
  ...
  const endpoint = `/auth/tenant/current/scopes/${scopeId}/actions${search}`;
  const result = await backendFetch<TenantScopeActionDirectoryResponse>(endpoint, {
    method: "GET",
...
export async function POST(
  ...
  const result = await backendFetch<TenantScopeActionDirectoryResponse>(
    `/auth/tenant/current/scopes/${scopeId}/actions`,
    {
      method: "POST",
      token: authResult.token,
      body
