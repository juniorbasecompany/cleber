...
| `event`   | idem | Evento operacional (`unity_id`, `age`, `location_id`, `item_id`, `action_id`). **`age`** é inteiro NOT NULL: valor na unidade de idade implícita do escopo (alinhado aos campos de idade do escopo). **Fato:** `unity_id` obrigatório. **Padrão (standard):** `unity_id` NULL. Com `unity_id` preenchido, `location_id` = `unity.location_id` e `item_id` ∈ `unity.item_id_list`. Em inglês técnico, traduza **«padrão»** neste sentido sempre **standard** (não *pattern*, *default*, etc.). |
...
- `.../actions/{action_id}/formulas` e `.../formulas/{formula_id}` (em `POST`/`PATCH`, a `statement` é validada com contrato de atribuição: exatamente um operador `=` de atribuição, `LHS` obrigatório `${field:id}` e `RHS` com `${field:id}` e `${input:id}`; todas as referências devem existir no escopo e a `RHS` passa por dry-run com [simpleeval](https://pypi.org/project/simpleeval/) e stubs coerentes com o `field.type` (incluindo `DATE`, `TIMESTAMP` e `TIMESTAMPTZ`, com persistência do resultado temporal em `result.text_value` em ISO). Códigos 422 estáveis: `formula_invalid_assignment`, `formula_invalid_target`, `formula_unknown_field_id`, `formula_expression_invalid`, com `step` no detalhe quando disponível. Implementação em `valora_backend/rules/`; cobertura em `tests/test_formula_statement_validate.py` e `tests/test_field_sql_formula.py`.)
...
- `POST .../scopes/{scope_id}/events/read-current-age` (lê os `result` já materializados conforme filtros opcionais `unity_id`, `location_id`, `item_id`; sem período na requisição)
- `POST .../scopes/{scope_id}/events/calculate-current-age` (apaga antes os `result` ligados aos eventos em escopo filtrado e recalcula; a **janela de idade** é por **unity**: eventos com idade inicial e final na mesma unidade; estado de idade atual é único por `unity_id`; cada ação executa na idade do evento (`event.age` em relação a `date(unity.creation_utc)`) e, se `action.is_recurrent = true`, volta a executar em cada idade seguinte até o próximo evento da mesma ação ou até o limite da idade final da janela; o fecho da janela ocorre **depois** de todas as fórmulas da ocorrência, quando a idade atual atinge a idade final. **Nota:** só entram **fatos** (`unity_id` preenchido). Eventos **padrão (standard)** não entram até extensão futura do motor.)
- `.../events/{event_id}/inputs` e `.../inputs/{input_id}`
- `.../events/{event_id}/results` e `.../results/{result_id}` (`result` segue o ERD atual: `unity_id`, `age`, `text_value`, `boolean_value`, `numeric_value`, `formula_id`, `formula_order`)
