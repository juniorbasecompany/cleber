...
- `POST .../scopes/{scope_id}/events/calculate-current-age` (apaga antes os `result` ligados aos eventos em escopo filtrado e recalcula; a **janela de idade** é por **unity**: eventos com idade inicial e final na mesma unidade; estado de idade atual é único por `unity_id`; cada ação executa no dia do evento e, se `action.is_recurrent = true`, volta a executar em cada dia seguinte até o próximo evento da mesma ação ou até o limite `date(unity.creation_utc) + idade final` da janela; o fecho da janela ocorre **depois** de todas as fórmulas da ocorrência, quando a idade atual atinge a idade final. **Nota:** só entram **fatos** (`unity_id` e `moment_utc` preenchidos). Eventos **padrão (standard)** não entram até extensão futura do motor.)
